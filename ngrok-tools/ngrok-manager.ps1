...
# Run this script with: powershell -ExecutionPolicy Bypass -File ngrok-manager.ps1
...
function Save-NgrokUrl {
    ...
    if (Test-Path $NgrokHistoryFile) {
        $history = Get-Content $NgrokHistoryFile | ConvertFrom-Json
    ...
    if ($history.Count -ge 10) {
        $history = $history | Select-Object -Last 9
    }
    $history += $entry
    
    $history | ConvertTo-Json -Depth 10 | Set-Content -Path $NgrokHistoryFile
    Write-Log "Saved ngrok URL to files"
}

function Update-RedirectService {
    param([string]$NewUrl)
    
    # Update the redirect HTML file
    $redirectHtml = @"
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    <div class="container">
        ...
        <p>If you are not redirected automatically, <a href="$NewUrl">click here</a></p>
...
"@
    ...
    Write-Log "Updated redirect.html with new URL: $NewUrl"
}
...
# Main execution
Write-Log "========================================="
Write-Log "Ngrok Manager Started"
...
if ($currentUrl) {
    ...
    Update-RedirectService -NewUrl $currentUrl
    ...
    Write-Host "=========================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "Share the redirect page or current URL with your users."
